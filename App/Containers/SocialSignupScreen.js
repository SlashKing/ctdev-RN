...
import { BackHandler, Keyboard, LayoutAnimation, ScrollView, Text, Image, View, Metrics } from "react-native";
...
class SocialSignupScreen extends React.Component {
  constructor(props){
    super(props);
    ...
    this.state={
      ...
      terms: false,
      ofAge: false,
      username : { val: "", error: null, touched:false},
      pass1 : { val: "", error: null, touched:false},
      pass2 : { val: "", error: null, touched:false},
      email: { val: "", error: null, touched:false},
    }
  }
	keyboardDidShowListener = {};
	keyboardDidHideListener = {};
 async componentWillMount(){
    await getCoordinates(this)
  ...
  _validate(value,key){
    ...
    switch(key){
      ...
      case "pass2":
        ...
        return str !== "" ? str : null;
      case "email":
        var emailTest = new RegExp("[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?")
  ...
  _switchTerms = (value)=> this.setState({terms:value})
	...
	render() {
	  let {username, pass1, pass2, email,latitude, date, longitude, terms, ofAge } = this.state
		return (
						...
						onPress={()=> {
						  if(username.touched && pass1.touched && pass2.touched && email.touched
						    ...
						    && latitude !== null && longitude !== null) {
						      this.props.registerUser(username.val, pass1.val, pass2.val, email.val, date,""+latitude, ""+longitude)
						  }else{
...
const mapDispatchToProps = dispatch => ({
	...
	registerUser: (user, pass1, pass2, email, birthday,latitude, longitude) => dispatch(LoginActions.registerRequest(user,pass1, pass2, email, birthday, latitude, longitude)),
});

export default connect(mapStateToProps, mapDispatchToProps)(SocialSignupScreen);
