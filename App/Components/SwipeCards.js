/*
  Originally copied from https://github.com/brentvatne/react-native-animated-demo-tinder
  Refactored by Nicholas LeBlanc to provide buttons
  as well as expose swipeActions and overlays to the Component
*/
'use strict';
...
const styles = StyleSheet.create({
  ...
  yupIcon:{
    color: 'rgba(59, 114, 161, 0.67)',
    fontSize:37,
  },
  superIcon:{
    color:'rgba(82, 147, 86, 0.67)',
    fontSize:37,
  }
});
...
export default class SwipeCards extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      card: this.props.cards[currentIndex[this.guid]],
	    swipeActions: this._getSwipeActions(),
	    overlays: this._getOverlays()
    };
  ...
  _forceLeftSwipe() {
    this.cardAnimation = Animated.timing(this.state.pan, {
	  ...
	  const cancelled = this.props.handleNope(this.state.card)
      if (status.finished) this._advanceState();
      else this._resetState();
      this.cardAnimation = null;
    }
  ...
  _forceUpSwipe() {
    this.cardAnimation = Animated.timing(this.state.pan, {
	  ...
	  const cancelled = this.props.handleMaybe(this.state.card)
      if (status.finished) this._advanceState();
      else this._resetState();
  ...
  _forceRightSwipe() {
    this.cardAnimation = Animated.timing(this.state.pan, {
	  ...
	  const cancelled = this.props.handleYup(this.state.card)
      if (status.finished) this._advanceState();
      else this._resetState();
      this.cardAnimation = null;
    }
      );
      console.log(currentIndex[this.guid], this.guid)
    this.props.cardRemoved(currentIndex[this.guid]);
  }
  _getSwipeActions(){
	return {
      swipeRight: this._forceRightSwipe.bind(this),
      swipeLeft: this._forceLeftSwipe.bind(this),
      swipeUp: this._forceUpSwipe.bind(this),
    }
  }
  _getOverlays(){
    return {
      renderNope : this.renderNope.bind(this),
      renderYup : this.renderYup.bind(this),
      renderMaybe: this.renderMaybe.bind(this)
    }
  }
  _goToNextCard() {
  ...
  componentDidMount() {
    console.log('componentDidMount SwipeCards' )
    this._animateEntrance();
  }
  ...
  renderStack() {
    ...
    return cards.map((card, i) => {
      ...
      let opacity = 0.95 + (0.05 / cards.length) * (i + 1);
      let lastOpacity = 0.95 + (0.05 / cards.length) * i;

      let scale = 0.95 + (0.05 / cards.length) * (i + 1);
      let lastScale = 0.95 + (0.05 / cards.length) * i;
      ...
      if (i + 1 === cards.length) {
        ...
        return <Animated.View key={`card${i}_top`}style={[animatedCardStyles]} {...this._panResponder.panHandlers}>
          {this.props.renderCard(this.state.card, this.state.overlays)}
        </Animated.View>;
      }
      return <Animated.View key={`card${i}`} style={style}>{this.props.renderCard(card)}</Animated.View>;
    });
  }
  ...
  renderCard() {
    ...
    return <Animated.View style={[animatedCardStyles]} {... this._panResponder.panHandlers}>
  ...
  renderButtons(){
	...
	let {pan ,enter, swipeActions} = this.state;
    let scale = enter.interpolate({
                           inputRange: [0, 1],
                           outputRange: [.93,1],
                         });
    let animatedButtonStyles = { position:'absolute',left:0,bottom:0,flex:1, transform: [{ scale: scale }], justifyContent:'flex-end' };
    let nopeScale = pan.x.interpolate({ inputRange: [-SWIPE_THRESHOLD, 0], outputRange: [0.8, 1], extrapolate: 'clamp' });
    let yupScale = pan.x.interpolate({ inputRange: [0, SWIPE_THRESHOLD], outputRange: [1, 0.8], extrapolate: 'clamp' });
    let animatedNopeStyles = { transform: [{ scale: nopeScale }] };
    let animatedYupStyles = { transform: [{ scale: yupScale }] };

    let maybeScale = pan.y.interpolate({ inputRange: [-SWIPE_THRESHOLD, -(SWIPE_THRESHOLD/2)], outputRange: [0.8, 1], extrapolate: 'clamp' });
    let animatedMaybeStyles = { transform: [{ scale: maybeScale }] };
    return <Animated.View key={"buttons"} style={[animatedButtonStyles,styles.buttonWrapper]}>
    			   <Animated.View style={animatedNopeStyles}>
    					 <Button style={[styles.drawerButton, styles.noButton]}
    				     onPress={()=>swipeActions.swipeLeft()}>
                 <Icon name="thumb-down" style={[styles.drawerIcon,styles.noIcon]}/>
               </Button>
             </Animated.View>
             <Animated.View style={animatedMaybeStyles}>
    					 <Button style={[styles.drawerButton, styles.superButton]}
    				     onPress={()=>swipeActions.swipeUp()}>
                 <Icon name="cannabis" style={[styles.drawerIcon, styles.superIcon]}/>
               </Button>
             </Animated.View>
    				 <Animated.View style={animatedYupStyles}>
    					 <Button style={[styles.drawerButton, styles.yesButton]}
    				     onPress={()=>swipeActions.swipeRight()}>
                     <Icon name="thumb-up" style={[styles.drawerIcon, styles.yupIcon]}/>
                </Button>
             </Animated.View>
           </Animated.View>;
  }
